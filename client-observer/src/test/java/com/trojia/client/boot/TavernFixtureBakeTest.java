...
/**
 * The M1 bake step: regenerates the committed {@code content/maps/baked/tavern_fixture.trojsav}
 * from the hand-authored {@code content/maps/src/tavern_fixture.tmx} on every test run, via
 * the same {@code tools} importer path {@code TavernImportRoundTripTest} (tools module)
 * exercises: parse map + tileset, load raws, {@link TiledWorldImporter#importWorld},
 * {@link TiledWorldImporter#toTrojSav}, write.
 *
 * <p>{@link TrojSav#writeTo} is byte-deterministic for identical map/raws content, so
 * rerunning this test is a no-op diff unless the fixture or the material raws actually
 * changed — that is the intended way to keep the baked file fresh. {@link FixtureWorldLoader}
 * is the only thing that reads the output at runtime, and it never depends on {@code tools}
 * (see {@code build.gradle.kts}: this dependency is {@code testImplementation}-only).
 *
 * <p>Also serves as Behavior 1's automated check (DoD1): the reloaded world's dimensions
 * are asserted against values derived independently from the map itself (footprint +
 * z-group count), not hardcoded.
 */
class TavernFixtureBakeTest {

    @Test
    void bakesTheTavernFixtureAndReloadsWithMapDerivedDimensions() throws IOException {
        ...
        Path outFile = RepoPaths.locate("content", "maps")
                .resolve("baked").resolve(FixtureWorldLoader.TAVERN_FILE);
        ...
        assertEquals(3, zLevelCount,
                "content/maps/README.md documents tavern_fixture.tmx as three z-levels (-1/+0/+1)");
    }
