...
printf '%s\n' "    .\\dist\\granadad.exe"
printf '%s\n' ""
printf '%s\n' "Share with a friend (ONE file, needs nothing else on their machine):"
printf '%s\n' "    dist\\granadad-standalone.exe"
printf '%s\n' "Prove it first:"
printf '%s\n' "    powershell -ExecutionPolicy Bypass -File .\\scripts\\verify-standalone.ps1"
printf '%s\n' ""
printf '%s\n' "Tweaked content\\ and want a fresh standalone WITHOUT a build? This gate"
printf '%s\n' "published the packer and the stripped base exactly so you never have to"
printf '%s\n' "wait for it again:"
printf '%s\n' "    powershell -ExecutionPolicy Bypass -File .\\scripts\\package-standalone.ps1"
printf '%s\n' "(seconds, host-only, no docker; code edits still need this gate.)"
printf '%s\n' ""
