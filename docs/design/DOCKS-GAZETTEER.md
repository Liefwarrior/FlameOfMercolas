...
- Faces: `cultist` (per the shipped `content/art/faces/face-archetypes.json` / FACES-SPEC.md §3.3 amendment, not `monk`) — **extension: `acolyte` tag-multiplier profile under `cultist` (placeholder)**.
...
- Faces: `noble` (per the shipped `content/art/faces/face-archetypes.json` / FACES-SPEC.md §3.3 amendment — prosperous/proprietor base, not `laborer`).
