...
| `src/docks_surface.tmx` | 192×128 (6×4 chunks), z-levels +0…+15. The full Docks ward surface per `docs/design/DOCKS-GAZETTEER.md` — all 25 keyed establishments (K01–K25), the waterfront (Long Quay + Pier Row + Beaching Strand + Harl's Yard shipyard + Outfall = 8 moorings/landings), the street grid (Tarwalk, Saltgate Rise, Ropewynd, Herring Lane, the Gullet), 4 residential Compounds, 45 hovels, and 3 sealed dungeon seams. Generated by `tools/scripts/gen_docks_surface.py` (deterministic — rerun it, never hand-edit the .tmx). |
...
### `docks_surface.tmx` (192×128 — 6×4 chunks, z:+0 … z:+15)

The whole Docks ward surface, authored from `docs/design/DOCKS-GAZETTEER.md` (the district
bible: §2.1 fixes the 192×128×16 footprint, §2.2 the waterfront, §2.3 the streets, §3 the
25 keyed establishments, §2.5 the Compound typology). Generated — not hand-drawn — by
`tools/scripts/gen_docks_surface.py`; edit the generator and rerun (byte-deterministic).

- **Z profile.** z:+0–+6 solid granite substrate, z:+7 dirt; authored content z:+8–+15.
  Three walk planes climb south from the harbor: **Band A** quayside (y 26–95, walk z:+11),
...
- **Establishments.** All K01–K25 per gazetteer §3 at deliberately variable footprints —
  from the 14×18 2-story Weighhouse and 66×10 Ropewalk down to the 8×8 Fenner's Pawn —
  plus an unkeyed Sailmaker's Loft. Squall's Bathhouse holds real depth-2 pool water on
  the z:+11 fluids sublayer; Brann's Chandlery has a z:+10 cellar (the gray ledger); the
  Drowned Hold is a floorless-in-places hulk over a flooded z:+10 undercellar.
...
- **Markers.** 190: `business_k01…k25_*` + unit/hovel/courtyard anchors, berth/pier/
  muster/patrol/exit anchors, 3 sealed **dungeon seams** (`dungeon_seam_outfall` z:+9,
  `dungeon_seam_drowned_hold` z:+10, `dungeon_seam_merles_floor` z:+11 — the starter
  dungeon is a separate later authoring effort; only these access stubs exist), and 19
  light sources graded per §7.2 (mast lamp 26 down to shrine candles 8; zero lights in
  the Gullet east end and on Wormwood Pier — design law).
- **Pipeline.** Baked to `baked/docks_surface.trojsav` by `DocksSurfaceBakeTest`
  ...
  (`DocksPopulation` — every K-site staffed, all 4 Compounds + 45 hovels occupied,
  dockworkers commuting to the berths/piers by day; headless proof:
...
`docks_surface.tmx`: the gazetteer fixes the street names, K-site roster and character,
z-profile, waterfront features, and placement logic (novel wins); **every concrete tile
coordinate is invented for v0, needs Eli's blessing** — same stance as `compound_block.tmx`.
Deviations from the authoring blueprint are marked `DEV:` in `gen_docks_surface.py`.
