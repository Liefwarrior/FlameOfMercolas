...
TEST_CASE("the owner's art pack loads when it is there") {
    ...
    // 109 authored regions covering 19 materials; every region is at least one
    // tile and several carry animation variants, so the store is well over 100.
    ...
    for (std::uint16_t m = 0; m < static_cast<std::uint16_t>(materialIds().size()); ++m) {
        for (std::size_t f = 0; f < kFaceKindCount; ++f) {
            ...
            CAPTURE(f);
            // Including the three facade materials, which carry no art of their
            // own and are aliased to the material they face.
